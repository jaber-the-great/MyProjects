...
for (item in data) {
  for (i in 1:n) {
    ...
    dataset[, 1] = toString(dataset[, 1])
    dataset[, 1] <- sub("^", "str", dataset[, 1] )
    dataset[, 2] = toString(dataset[, 2])
...
library(MASS)
write.matrix(T,file="/Users/katieturnlund/Workspace/CS263_Jaber_Katie/Code/Linear Reg R/R_results_dataClean.csv")